        ...
        NavigationStack {
            GlassEffectContainer {
                ZStack {
                    ...
                    ScrollView {
                        VStack(spacing: 24) {
                            // Provider Selection with Fluid Morphing
                            VStack(alignment: .leading, spacing: 12) {
                                ...
                                VStack(spacing: 8) {
                                    ForEach(ALL_PROVIDERS) { provider in
                                        ...
                                        .glassEffect()
                                        .glassEffectID("provider-\(provider.id)", in: glassNamespace)
                                        .interactive()
                                    }
                                }
                                .padding(.horizontal, 16)
                            ...
                            // Model Selection with Interactive Glass
                            VStack(alignment: .leading, spacing: 12) {
                                Text("Model")
                                    .font(.system(size: 14, weight: .semibold))
                                    .foregroundStyle(.secondary)
                                ...
                                VStack(spacing: 8) {
                                    ForEach(selectedProvider.models) { model in
                                        ...
                                        .glassEffect()
                                        .glassEffectID("model-\(model.id)", in: glassNamespace)
                                        .interactive()
                                    }
                                }
                                .padding(.horizontal, 16)
